...
- `php artisan test` (or `composer test`) — test suite (clears config cache first).
- `php artisan test --filter=TestName` or `php artisan test path/to/Test.php` — run a single test.
...
- **WaterPoint (titik air)** is GIS-trajectory data: no single lat/lng — it stores `start_latitude/longitude`, `end_latitude/longitude`, `recommend_latitude/longitude`, `direction`, `documentation_photo`, `interpretation_photo`. Map markers use the `recommend_*` pair. Coordinates are not shown to the public.
